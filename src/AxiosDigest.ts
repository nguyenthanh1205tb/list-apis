...
export class AxiosDigest {
  ...
  public async get(path: string, config?: AxiosRequestConfig) {
    let wwwAuth: any
    try {
      wwwAuth = await this.axios.get(path, config)
    } catch (r) {
      wwwAuth = await this.getWwwAuth(r)
    }
    const c = this.getAuthHeader(wwwAuth, 'GET', path, config)
    return await this.axios.get(path, c)
  }
  ...
  private getAuthHeader(
    ...
    const cnonce: string = Math.random().toString(32).substring(2)
    const nc = '0001'
    const qop = 'auth'
